...
elseif ($_REQUEST['process'] === 'generate') {
   ...
   fetchVar($css_filename, 'cssfilename', "icons.css");
   fetchVar($css_template, 'csstemplate', ".icon-{ICON} { background: url('../icons/{FILE}') {X}px {Y}px; }");
   fetchVar($out_file_template, 'imagefilename', "itemsprites_{FILENUM}");
...
TPL;
   ...
   set_progress("Scanning dragitem directory for source icons.".$outputHeader);

   //figure out which drag items we are going to process
   //by examining the dragitems directory. We're assuming
   //there isn't a bunch of garbage in the directory
   $files_unsorted = glob('dragitems/*.{dds}', GLOB_BRACE);
   
   /*$files_unsorted = array(
      'dragitems/dragitem1.dds',
      'dragitems/dragitem2.dds');*/

   //ERROR CHECK DRAGITEM FILE LIST
   ...
   {
      $textout = "You are missing some drag item files! Scan the files below and replace the missing file in the ./dragitems/ directory<br/><br/>";
      for ($i = 1; $i <= $max_index; $i++) {
         $textout .= "./dragitems/dragitem$i.dds &#8680; ";
   ...
   //PROCESS DRAGITEM FILES INTO NEW IMAGES AND CSS  
   $starttime = time();
   $last_output = time();
   foreach($files as $file) {
      ...
      for ($source_x = 0; $source_x < 240; $source_x += ICON_DIM) {
         for ($source_y = 0; $source_y < 240; $source_y += ICON_DIM) {
            ...
            //cut from the drag item and paste to our new sprite sheet
...
Input Files: dragitem%s.dds to dragitem%s.dds

Output Files: %s to %s
              %s total image files

Output images: %s x %s items
               %s x %s pixels
...
TPL;
   ...
   //create zip file
   $zip_filename = "dragitems_".time()."_".$outrows."x".$outcols."_".$out_file_type.".zip";
